function ConsolidatedTAGUI7c()

%% Create the main UI figure
fig = uifigure('Position', [100, 100, 600, 800], 'Name', 'Thermal Data Analysis ConsolidatedTAGUI7c');
%% Banner (Top)
...
uilabel(panelSNR, 'Position', [220, 7, 39, 30], 'Text', 'Frame/Pixel:','WordWrap', 'on');
snrVarMethodDropdown = uidropdown(panelSNR, 'Position', [260, 10, 80, 20], 'Items', {'Per Frame', 'Per Pixel'}, 'Value', 'Per Frame', 'Tag', 'snrVarMethodDropdown');
...
snrXAxisUnitDropdown = uidropdown(panelSNR, 'Position', [350, 10, 80, 20], 'Items', {'Frame', 'Time (s)'}, 'Value', 'Frame', 'Tag', 'snrXAxisUnitDropdown');
%% Panel 6: Heatmaps
panelHeatmaps = uipanel(fig, 'Title', 'Heatmaps', 'Position', [20, 440, 560, 60], 'FontWeight', 'bold', 'FontSize', 12);
uilabel(panelHeatmaps, 'Position', [5, 5, 60, 30], 'Text', 'SNR Method:','WordWrap', 'On');
snrMethodDropdown = uidropdown(panelHeatmaps, 'Position', [55, 5, 60, 30], 'Items', {'Mean-Based SNR', 'Variance-Based SNR', 'Hybrid SNR'}, 'Value', 'Mean-Based SNR', 'Tooltip', 'Select SNR computation method', 'Tag', 'snrMethodDropdown', 'ValueChangedFcn', @updateHeatmap);
btnGenerateHeatmaps = uibutton(panelHeatmaps, 'Position', [480, 5, 70, 30], 'Text', 'Heatmaps', 'ButtonPushedFcn', @generateHeatmaps);
uilabel(panelHeatmaps, 'Position', [130, 5, 100, 30], 'Text', 'Distance Measurement:', 'WordWrap', 'On');
distanceMeasurementDropdown = uidropdown(panelHeatmaps, 'Position', [215, 5, 60, 30], 'Items', {'Manual Distance: Frame', 'Manual Distance: SNR Heatmap', 'Manual Distance: Variance Heatmap', 'Manual Distance: Damage Indicator Heatmap','Auto Distance: Frame', 'Auto Distance: SNR Heatmap', 'Auto Distance: Variance Heatmap', 'Auto Distance: Damage Indicator Heatmap'}, 'Value', 'Manual Distance: Frame', 'Tag', 'distanceMeasurementDropdown', 'Tooltip', 'Select distance measurement type and target', 'ValueChangedFcn', @performDistanceMeasurement);
scaleUnitsCheckbox = uicheckbox(panelHeatmaps, 'Text', 'mm', 'Position', [285, 7, 90, 30], 'Value', true, 'Tooltip', 'Toggle between pixels and millimeters for image scaling', 'Tag', 'scaleUnitsCheckbox');

% Colour scale controls
colourScaleDropdown = uidropdown(panelHeatmaps, 'Position', [330, 10, 50, 20], 'Items', {'Auto', 'Fixed'}, 'Value', 'Auto','Tag', 'colourScaleDropdown', 'Tooltip', 'Select automatic or fixed colour limits');
uilabel(panelHeatmaps, 'Position', [390, 20, 30, 20], 'Text', 'Min:');
colourMinField = uieditfield(panelHeatmaps, 'numeric',  'Position', [420, 18, 35, 20], 'Value', 0, 'Tag', 'colourMinField', 'Tooltip', 'Fixed colour scale minimum');
uilabel(panelHeatmaps, 'Position', [390, 3, 30, 20], 'Text', 'Max:');
colourMaxField = uieditfield(panelHeatmaps, 'numeric', 'Position', [420, 3, 35, 20],  'Value', 30, 'Tag', 'colourMaxField', 'Tooltip', 'Fixed colour scale maximum');
...
%% Nested Functions
    ...
    function calcDiffusivity(~, ~)
        ...
        nFrames = numel(tempProfile);

        % Sync UI totalFrames field
        totalFramesField = findobj(fig,'Tag','totalFramesField');
        if ~isempty(totalFramesField)
            totalFramesField.Value = nFrames;
        end

        % Time vector (ONLY from data)
        time = (0:nFrames-1) / frameRate;
        dataEndTime = time(end);

        %% ---- User inputs ----
        pulseDuration  = getFieldValue(findobj(fig,'Tag','pulseDurationField'), 3000) / 1000; % s
        pulseStartTime = getFieldValue(findobj(fig,'Tag','startTimeField'), 0.1);             % s
        timeInterval   = getFieldValue(findobj(fig,'Tag','timeIntervalField'), 6);            % s
        ...
        %% ---- Pulse timing  ----
        pulseStartTime = max(pulseStartTime, 0);

        pulseEndTime = pulseStartTime + pulseDuration;

        % Clamp pulse end to data duration
        pulseEndTime = min(pulseEndTime, dataEndTime);

        % Frame indices
        pulseStartFrame = max(1, round(pulseStartTime * frameRate) + 1);
        pulseEndFrame   = min(round(pulseEndTime   * frameRate) + 1, nFrames);

        %% ---- Sample window ----
        sampleEndTime  = pulseStartTime + timeInterval;
        sampleEndTime  = min(sampleEndTime, dataEndTime);
        sampleEndFrame = min(round(sampleEndTime * frameRate) + 1, nFrames);

        %% ---- Hard validation ----
        if pulseStartFrame >= nFrames
            uialert(fig, 'Pulse start time exceeds data duration.', 'Error');
            return;
        end

        if pulseEndFrame <= pulseStartFrame
            uialert(fig, 'Pulse duration exceeds data length.', 'Error');
            return;
        end

        if sampleEndFrame <= pulseStartFrame
            uialert(fig, 'Time interval too short.', 'Error');
            return;
        end

        %% ---- Background subtraction ----
        bgFrames = min(bgFrames, nFrames);
        ...
        alpha = (sourceDepth^2) / (4 * delta_t) * ...
            (deltaT / max(tempProfileSmoothed));
        ...
        % Pulse ON shading (never appears in legend)
        yLimits = [min(tempProfileSmoothed) max(tempProfileSmoothed)];
        patch( ...
            [pulseStartTime pulseEndTime pulseEndTime pulseStartTime], ...
        ...
        % ---- DATA LINES  ----
        hRaw = plot(time, tempProfile, 'b-', 'LineWidth', 1.2);
        hBg  = plot(time, tempProfileSmoothed, 'r-', 'LineWidth', 1.5);

        % Markers
        hT1 = plot(time(pulseStartFrame), T1, 'go', 'MarkerSize', 10, 'LineWidth', 2);
        hT2 = plot(time(sampleEndFrame),  T2, 'gx', 'MarkerSize', 10, 'LineWidth', 2);

        xline(pulseStartTime, '--k', 'Pulse Start');

        xlabel('Time (s)');
        ylabel('Average Temperature Rise');
        title('Frame-Averaged Temperature Rise');

        % ---- EXPLICIT LEGEND ----
        legend([hRaw, hBg, hT1, hT2], ...
            {'Raw', 'Smoothed', 'T_1', 'T_2'}, ...
            'Location','best');
...
%% MP4
    ...
    function createMP4(~, ~)
        ...
        % Apply 3D filters once (as you currently do)
        disp('Applying 3D filters to all selected frames...');
        ...
        % ---- Build ONE hidden figure/axes ONCE ----
        figTemp = figure('Visible', 'off', 'Color', 'w');
        ax = axes(figTemp);

        % Prepare first frame image
        frame0 = filteredData(:,:,1);
        frame0 = flipud(frame0);

        % normalize to 0..1 for display
        mn0 = min(frame0(:)); mx0 = max(frame0(:));
        if mx0 > mn0
            frame0n = (frame0 - mn0) / (mx0 - mn0);
        else
            frame0n = zeros(size(frame0));
        end

        [h, w] = size(frame0n);

        if useMillimeters
            xAxis = (0:w-1) * pixelSize;
            yAxis = (0:h-1) * pixelSize;

            hImg = imagesc(ax, xAxis, yAxis, frame0n);
            xlabel(ax, 'X (mm)', 'FontName', 'Arial', 'FontSize', 14);
            ylabel(ax, 'Y (mm)', 'FontName', 'Arial', 'FontSize', 14);
        else
            hImg = imagesc(ax, frame0n);
            xlabel(ax, 'X (Pixels)', 'FontName', 'Arial', 'FontSize', 14);
            ylabel(ax, 'Y (Pixels)', 'FontName', 'Arial', 'FontSize', 14);
        end
        ...
        % Keep axes/ticks (since you want them), but avoid expensive changes each loop:
        grid(ax, 'off');      % grid is expensive in videos; enable if you truly need it
        box(ax, 'on');

        % Use a text overlay instead of updating title (title redraw is slower)
        txt = text(ax, 0.02, 0.98, '', 'Units', 'normalized', ...
            'Color', 'w', 'FontSize', 12, 'FontWeight', 'bold', ...
            'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', ...
            'BackgroundColor', 'k', 'Margin', 2);

        % Force a consistent axis range so it never “jumps”
        if useMillimeters
            xlim(ax, [xAxis(1) xAxis(end)]);
            ylim(ax, [yAxis(1) yAxis(end)]);
        else
            xlim(ax, [1 w]);
            ylim(ax, [1 h]);
        end
        ...
        % ---- Write frames (ONLY update CData + text) ----
        hWait = waitbar(0, 'Writing MP4...');
        ...
        for i = 1:frameCount
            ...
            mx = max(frameToWrite(:));
            if mx > mn
                frameToWrite = (frameToWrite - mn) / (mx - mn);
            else
                frameToWrite = zeros(size(frameToWrite));
            end

            % Update image only (fast)
            hImg.CData = frameToWrite;

            % Update overlay text only (fast)
            originalFrameIndex = startIndex + (i-1) * frameInterval;
            txt.String = sprintf('Frame %d', originalFrameIndex);
        ...
        uialert(fig, sprintf('MP4 created successfully:\n%s', videoFilename), 'Success');
...
%% SNR and Variance
    ...
    function calculateSNRandVariance(~, ~)
        ...
        % ---- Read x-axis unit dropdown ----
        xUnitDropdown = findobj(fig, 'Tag', 'snrXAxisUnitDropdown');
        if isempty(xUnitDropdown)
            xUnit = 'Frame';
        else
            xUnit = xUnitDropdown.Value;  % 'Frame' or 'Time (s)'
        end
        ...
        pulseStartFrame = max(1, round(pulseStartTime * frameRate) + 1);
        pulseEndFrame   = min(round((pulseStartTime + pulseDuration) * frameRate) + 1, z_dim);

        % ---- X values ----
        frames = 1:z_dim;
        time_s = (frames - 1) / frameRate;

        switch xUnit
            case 'Time (s)'
                xVals = time_s;
                xLabelStr = 'Time (s)';
                pulseStartX = (pulseStartFrame - 1) / frameRate;
                pulseEndX   = (pulseEndFrame   - 1) / frameRate;
            otherwise
                xVals = frames;
                xLabelStr = 'Frame Index';
                pulseStartX = pulseStartFrame;
                pulseEndX   = pulseEndFrame;
        end
        ...
        % SNR first (left axis)
        yyaxis left;
        hSNR = plot(xVals, snr_values, 'o-', 'DisplayName', 'SNR');
        ylabel('SNR (dB)');
        hold on;

        % Pulse band AFTER SNR plot so it spans full height
        yl = ylim;
        if pulseEndX > pulseStartX
            hBand = patch( ...
                [pulseStartX pulseEndX pulseEndX pulseStartX], ...
                [yl(1) yl(1) yl(2) yl(2)], ...
                [0.7 0.7 0.7], ...
                'EdgeColor', 'none', ...
                'FaceAlpha', 0.35, ...
        ...
        % Pulse start line
        xline(pulseStartX, '--k', 'Pulse Start', ...
            'LabelVerticalAlignment','bottom', ...
            'HandleVisibility','off');

        % Variance (right axis)
        yyaxis right;
        hVar = plot(xVals, variance_values, 'x-', 'DisplayName', 'Variance');
        ylabel('Variance');

        % Force variance axis to start at 0
        yl = ylim;
        ylim([0, yl(2)]);
        ...
        % Lock x-limits so the patch/line cannot stretch the axis
        xlim([xVals(1) xVals(end)]);

        legend([hSNR, hVar], {'SNR', 'Variance'}, 'Location', 'best');
        grid on;
        hold off;
    ...
    function applyColourLimits(ax)
        colourScaleDropdown = findobj(fig, 'Tag', 'colourScaleDropdown');
        colourMinField = findobj(fig, 'Tag', 'colourMinField');
        colourMaxField = findobj(fig, 'Tag', 'colourMaxField');
    
        if isempty(colourScaleDropdown) || isempty(colourMinField) || isempty(colourMaxField)
            return;
        end
    
        if strcmp(colourScaleDropdown.Value, 'Fixed')
            cMin = colourMinField.Value;
            cMax = colourMaxField.Value;
    
            if cMax <= cMin
                uialert(fig, 'Colour scale maximum must be greater than minimum.', 'Invalid Colour Limits');
                return;
            end
